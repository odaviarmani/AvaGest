...
const getBackgroundColorDark = (percentage: number) => {
    const hue = (percentage / 100) * 120;
    return `hsl(${hue}, 40%, 10%)`;
};
...
export default function TestCard({ test, onEdit, onDelete, onDuplicate, onSelectForComparison, isSelectedForComparison }: TestCardProps) {
    ...
    const bgColor = getBackgroundColor(successPercentage);
    const bgColorDark = getBackgroundColorDark(successPercentage);


    return (
        <Card 
            className="flex flex-col transition-all duration-200 hover:shadow-lg aspect-square"
            style={{ '--bg-light': bgColor, '--bg-dark': bgColorDark } as React.CSSProperties}
        >
            <div className="bg-[var(--bg-light)] dark:bg-[var(--bg-dark)] rounded-t-lg flex-1 flex flex-col">
